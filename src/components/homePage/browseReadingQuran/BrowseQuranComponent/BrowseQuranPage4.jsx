...
function BrowseQuranPage4() {
  ...
  useEffect(() => {
    ...
    handleJuzSelection();
  }, [selectedJuz, type, data, dispatch]);
  ...
  const handleNext = async () => {
    ...
    if (rightPage < 604) {
      ...
      try {
        const pageData = await dispatch(fetchSurahPage({ type: "page", selectedNumber: `${isMobile ? newPage : newRight}` })).unwrap()
  ...
  return (
    <div className={`w-full sm:w-[64%] md:w-[67%] lg:w-2/3 xl:w-3/4 2xl:w-[77%] min-h-[calc(100vh-80px)] flex flex-col gap-3 items-center justify-start rounded-md  ${darkMode ? "bg-[#000] border-[#5d5d5d]" : ""}`}>

      <h1 className={`w-full flex justify-start items-center gap-2 py-1 px-2 text-md sm:text-xl md:text-xl font-bold  border rounded-md ${darkMode ? "border-[#5d5d5d] text-[#fff]" : "border-[#ddd] text-[#22a5ad] bg-white "}`} dir='rtl'>
        <span> {surahName}</span>   /
      ...
      {/* Select Boxes */}
      <div className={`w-full flex flex-wrap gap-1 sm:flex-nowrap sm:gap-2 justify-between  border  p-1 rounded-md ${darkMode ? "border-[#5d5d5d]" : "border-[#ddd] bg-white"}`} dir='rtl'>

        <select value={selectedSurah} onChange={(e) => { handleSelection(e); setType("page"); }} className={`w-full lg:w-auto px-2 py-1 rounded-md outline-none border overflow-y-scroll scrollbar-thin scrollbar-thumb-white scrollbar-track-black ${darkMode ? "bg-[#000] text-[#fff] border-[#5d5d5d]" : "border-[#ccc] bg-[#f1f1f1]"}`}>
        ...
        <select value={juzNumber ? juzNumber : selectedJuz || ""} onChange={(e) => { setSelectedJuz(Number(e.target.value)); setType("juz"); }} className={`px-4 py-1 rounded-md outline-none border ${darkMode ? "bg-[#000] text-[#fff] border-[#5d5d5d]" : "bg-[#f1f1f1] border-[#ccc]"}`}>
      ...
      {/* Ayahs Display (Image) */}
      <div className={`w-full flex justify-center items-center bg-white shadow-2xl rounded-md border-7  ${darkMode ? "bg-[#000] text-[#fff]" : "border-[#22a5ad]"}`}>
        {
          loadingAyahs ? (
            ...
            <>
              ...
              <div className="hidden xl:flex w-full">
                ...
                <img
                  src={`https://cdn.qurango.net/Sura2/files/mobile/${rightPage + 1}.jpg`}
                  alt={`Quran Page ${rightPage}`}
                  className="w-1/2 h-auto object-contain rounded-md"
                />
                <img
                  src={`https://cdn.qurango.net/Sura2/files/mobile/${leftPage + 1}.jpg`}
      ...
      {/* Navigation Buttons */}
      <div className={`w-full flex justify-center items-center gap-9 rounded-md py-1 mb-1  ${darkMode ? "bg-black border border-[#5d5d5d] py-2" : "bg-[#f1f1f1]"}`}>
        <button onClick={handleBack} className={`flex justify-center items-center gap-2 px-5 py-1 sm:py-2 md:px-6 rounded-lg font-semibold 
          ${darkMode ? "bg-[#000] border border-[#5d5d5d] text-[#fff] hover:bg-[#5d5d5d]" : "bg-[#ddd]"}
           ${leftPage === 1 || leftPage === 2 ? ' text-gray-500 cursor-not-allowed'
            : 'hover:bg-[#22a5ad] hover:text-white  text-[#22a5ad] cursor-pointer '}`}>
        ...
        <button onClick={handleNext} className={`flex justify-center items-center gap-2 px-5 py-1 sm:py-2 md:px-6 rounded-lg font-semibold duration-300  
          ${darkMode ? "bg-[#000] border border-[#5d5d5d] text-[#fff] hover:bg-[#5d5d5d]" : "bg-[#ddd]"} 
          ${rightPage === 604 || rightPage === 603 ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'hover:bg-[#22a5ad] hover:text-white  text-[#22a5ad] cursor-pointer '}`}>
...
export default BrowseQuranPage4;
